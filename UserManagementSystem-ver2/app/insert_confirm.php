    ...
    <body>
        ...
        if (!$mode == "CONFIRM") {
            ...
            if (!in_array(null, $confirm_values, true)) {
                if (!checkdate($confirm_values['month'], $confirm_values['day'], $confirm_values['year'])) {
                ...
                foreach ($confirm_values as $key => $value) {
                    if ($value == null) {
                        ...
                        echo 'が未記入です<br>';
                        //うるう年等、存在しない日付をcheckdateを用いてチェックしています。
                        //存在しなかった下記を表示後、トップへ戻るか前画面に戻るよう選択肢を表示。
                    }
                }
                if (!checkdate($confirm_values['month'], $confirm_values['day'], $confirm_values['year']) && !empty($birthday)) {
                    echo '<br>正しい日付を入力してください';
                }
            }
            ?>
